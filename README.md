...
[GitHub-hosted runners](https://github.com/actions/runner-images) include Eclipse Adoptium's Temurin JDKs in their tool cache. To avoid downloading a JDK and reduce setup time, prefer `distribution: 'temurin'` with the default `check-latest: false` when a cached version meets your needs. Available versions depend on the selected runner image.
